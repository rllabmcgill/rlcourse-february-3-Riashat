...
def Sigmoid_Behaviour_Policy(Q, sigmoid_prob, nA):
...
def mc_control_importance_sampling(env, num_episodes, discount_factor=1.0, epsilon=0.2):    
    ...
    P = 1 / (1 + np.exp(-all_action_values))
    sigmoid_prob =( 1 / (1 + np.exp(-all_action_values)) )/  ( np.sum((1 / (1 + np.exp(-all_action_values))), axis=0) ) 

    behavior_policy = Sigmoid_Behaviour_Policy(Q, sigmoid_prob, env.action_space.n)
